...
def writeData():
    ...
    c.execute(
        "CREATE TABLE character (id INTEGER PRIMARY KEY AUTOINCREMENT, npcNameLocal TEXT, sex BOOL, type TEXT, topic TEXT, text TEXT, npcNameCode TEXT, language TEXT, fileName TEXT, audioURL TEXT)"
    )
    ...
    with open("./db/result.json", "r", encoding="utf-8") as f:
        ...
        for i in data:
            if data.get(i).get("language") == "CHS":
                ...
                # 转换url
                chadto.audioURL = "https://api.csu.st/file/" + "/".join(
                    chadto.fileName.split("\\")[1:]
                ).replace(".wem", ".ogg")
                # 查表填入性别
                type_one_list = [
                    "荧",
