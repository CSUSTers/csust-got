...
func NewCustomAgent(ctx context.Context, cfg *CustomAgentConfig) (*CustomAgent, error) {
	...
	for _, t := range cfg.Tools {
		info, err := t.Info(ctx)
		if err != nil {
			zap.L().Warn("agentv3/loop: failed to fetch tool info, skipping",
				zap.String("agent", cfg.Name), zap.Error(err))
			continue
		}
		if _, exists := seenNames[info.Name]; exists {
			zap.L().Warn("agentv3/loop: duplicate tool name, keeping first",
		...
		if it, ok := t.(tool.InvokableTool); ok {
			...
			zap.L().Warn("agentv3/loop: tool is not invokable",
...
func (a *CustomAgent) executeToolCall(ctx context.Context, tc schema.ToolCall) *schema.Message {
	...
	if !ok {
		zap.L().Warn("agentv3/loop: model called unknown tool",
