...
func (m *McpManager) GetToolsFromConfig(ctx context.Context, cfgs []*config.ToolServerConfig) ([]tool.BaseTool, error) {
	...
	for _, cfg := range cfgs {
		...
		if err != nil {
			zap.L().Error("agentv3/mcp: failed to get tools from server",
...
func (m *McpManager) getToolsFromMCP(ctx context.Context, cfg *config.ToolServerConfig) ([]tool.BaseTool, error) {
	...
	if len(trimmedNames) > 0 {
		...
		if len(missing) > 0 {
			zap.L().Warn("agentv3/mcp: configured tool names not found on server",
				zap.String("url", cfg.Url),
	...
	zap.L().Info("agentv3/mcp: discovered MCP tools",
		zap.String("url", cfg.Url),
...
func (m *McpManager) getToolsFromMcpo(ctx context.Context, cfg *config.ToolServerConfig) ([]tool.BaseTool, error) {
	...
	for _, entry := range cfg.Tools {
		...
		if err != nil {
			zap.L().Error("agentv3/mcp: failed to discover MCPO toolset",
				zap.String("url", cfg.Url),
	...
	if len(allTools) > 0 {
		...
		zap.L().Info("agentv3/mcp: discovered MCPO tools",
			zap.String("url", cfg.Url),
...
func (m *McpManager) Close() {
	...
	for url, cli := range m.clients {
		if err := cli.Close(); err != nil {
			zap.L().Warn("agentv3/mcp: failed to close MCP client",
				zap.String("url", url),
				zap.Error(err),
			)
		}
