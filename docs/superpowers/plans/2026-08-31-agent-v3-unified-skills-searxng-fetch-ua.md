...
- [ ] **Step 2: Verify Rust format, lint, focused boundaries, and full crate**
  ...
  function Assert-OutsideImplementationHunks {
    ...
    $addedResult = Invoke-CapturedProcess $GitExecutable @('-c', 'core.autocrlf=false', 'diff', '--name-only', '--diff-filter=A', $PlanBaseSHA, '--') $RepoRoot
    ...
    foreach ($diagnostic in $Diagnostics) {
      ...
      foreach ($location in $locations) {
        ...
        $diffResult = Invoke-CapturedProcess $GitExecutable @('-c', 'core.autocrlf=false', 'diff', '--unified=0', '--no-color', '--no-ext-diff', $PlanBaseSHA, '--', $location.Path) $RepoRoot
  ...
  $planDiffCheck = Invoke-CapturedProcess $gitExecutable @('-c', 'core.autocrlf=false', 'diff', '--quiet', $planBaseSha, '--', $planPath) $repoRoot
...
- [ ] **Step 6: Assemble acceptance, then hand off A–F commits and push to the authorized orchestrator**
  ...
  4. `Go matrix`: focused/race/full/build results plus the `golangci-lint` current exit. Include the four completeness/read-only overrides, current and baseline content-manifest receipts, confirmation that every captured plan-binding/ownership `git diff` used only the process-local `-c core.autocrlf=false` override while retaining fail-closed stderr, and, if nonzero, tool version/platform, exact current and baseline diagnostic identities/counts, exact ordinal four-identity baseline-dictionary equality, strict multiset-subset result, new-file/hunk exclusion, and removed/surviving baseline items under the label `zero new lint relative to same-tool PLAN_BASE`.
