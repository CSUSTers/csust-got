...
func FakeBanBase(kf KeyFunction, exec BanExecutor) module.Module {
	banner := func(ctx context.Context, update tgbotapi.Update, bot *tgbotapi.BotAPI) {
		...
		ban := func() bool {
			return ctx.GlobalClient().Set(ctx.WrapKey(kf(banTarget)), "banned", banTime).Err() == nil
		}
		spec := BanSpec{
			BanTarget:  banTarget,
	...
	interrupter := func(ctx context.Context, update tgbotapi.Update, bot *tgbotapi.BotAPI) module.HandleResult {
		...
		_, err := ctx.GlobalClient().Get(ctx.WrapKey(kf(target))).Result()
		if err != redis.Nil {
...
func FakeBan(tgbotapi.Update) module.Module {
	keyFunction := func(user *tgbotapi.User) string {
		return fmt.Sprintf("%d:banned", user.ID)
	}
	return FakeBanBase(keyFunction, generateTextWithCD)
}
